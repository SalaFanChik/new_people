...
from aiogram.types import Message, ChatMemberUpdated, MessageEntity, User
...
async def process_sg_text(message: Message, state: FSMContext, session: AsyncSession) -> None:
...
async def cmd_help(message: Message):
    await message.answer(_("Этот бот создан чтобы информировать о предстоящих мероприятиях."))
